...
const LoanPaymentStatus = z.enum(["PENDING", "PAID", "OVERDUE"], {
  invalid_type_error: "Durum geçersiz.",
});

export const loanPaymentCreateSchema = z
  .object({
...
const zOptDate = z.preprocess(
  (v) => (v === undefined || v === null ? undefined : new Date(v as any)),
  z.date({ invalid_type_error: "Geçerli bir tarih giriniz." })
).optional();
...
export const loanPaymentUpdateSchema = z
  .object({
    ...
    description: z.string().trim().max(2000, "Açıklama 2000 karakteri aşamaz.").optional(),
  })
  .superRefine((data, ctx) => {
    const hasParts =
